...
import { collection, query, getDocs, addDoc, updateDoc, doc, where, getDoc, setDoc } from 'firebase/firestore';
                    ...
                    {[1, 2, 3, 4, 5, 6, 7, 8].map(sem => (
                      <div key={sem} className="space-y-1">
                        ...
                        <div className="relative">
                          ...
                          <input 
                            type="number"
                            className="w-full pl-7 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            <form onSubmit={handleAddFee} className="space-y-4">
              <div>
                ...
                >
                  ...
                  {students.map(s => <option key={s.id} value={s.uid}>{s.name}</option>)}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
