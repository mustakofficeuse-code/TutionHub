        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-[11px] font-black text-wa-teal uppercase tracking-[0.4em]">MISSION REPOSITORY</span>
            </div>
            ...
            <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] mt-3 ml-1">Manage and track student submissions for active courses</p>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            assignments.map((a) => {
              ...
              return (
                <div key={a.id} className="bg-white dark:bg-[#202c33] rounded-[2.5rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-white/5 overflow-hidden group hover:border-wa-teal transition-all duration-500 hover:-translate-y-2">
                  <div className="p-8 space-y-5">
                    ...
                    <div>
                      <h3 className="font-black text-slate-800 dark:text-white text-xl line-clamp-1 italic uppercase tracking-tight">{a.title}</h3>
                      <p className="text-[11px] font-black text-wa-teal uppercase tracking-[0.2em] mt-2 block">
                        {a.subject} <span className="text-slate-300 dark:text-slate-700 mx-2">|</span> {dept?.name || 'ALL DEPARTMENTS'}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 text-[12px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest pb-6">
                      <Calendar className="w-4 h-4 text-wa-teal" />
                      <span>DUE: {new Date(a.dueDate).toLocaleDateString()}</span>
                    </div>
                    <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-white/5">
                      <span className="text-[10px] font-black text-wa-teal bg-wa-teal/10 dark:bg-wa-teal/20 px-3 py-1.5 rounded-lg uppercase tracking-widest border border-wa-teal/10">
                        {subCount} SUBMISSIONS
                      </span>
                      <button 
                        onClick={() => setViewSubmissions(a)}
                        className="text-slate-800 dark:text-[#e9edef] text-[11px] font-black flex items-center gap-2 hover:text-wa-teal transition-colors tracking-widest uppercase"
                      >
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto border border-slate-100 dark:border-slate-800">
            <h3 className="text-2xl font-black text-slate-800 dark:text-white mb-8 uppercase italic tracking-tight">Create New Assignment</h3>
            <form onSubmit={handleAdd} className="space-y-6">
              ...
              <div>
                <label className="block text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2.5 ml-1">Description</label>
                <textarea
                  rows={3}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2.5 ml-1">Subject</label>
                  <input
                    type="text"
                    required
                    className="w-full px-6 py-5 bg-[#f8f9fa] dark:bg-[#111b21] border border-transparent focus:border-wa-teal/30 focus:ring-4 focus:ring-wa-teal/5 rounded-2xl text-base font-black text-slate-800 dark:text-white transition-all outline-none"
                ...
                <div>
                  <label className="block text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2.5 ml-1">Due Date</label>
                  <div className="relative group">
                    <input
                      type="date"
                      required
                      className="w-full px-6 py-5 bg-[#f8f9fa] dark:bg-[#111b21] border border-transparent focus:border-wa-teal/30 focus:ring-4 focus:ring-wa-teal/5 rounded-2xl text-base font-black text-slate-800 dark:text-white transition-all outline-none pr-14"
                      value={dueDate}
              ...
              <div>
                <label className="block text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2.5 ml-1">Target Department</label>
                <select
                  required
