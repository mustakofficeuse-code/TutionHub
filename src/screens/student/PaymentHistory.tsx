          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                ...
                <span className="text-xs font-bold text-wa-teal bg-wa-teal/10 px-3 py-1 rounded-full  tracking-normal">{payments.length} Records</span>
              </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="font-bold text-slate-900 dark:text-[#e9edef] text-xl tracking-normal">₹{Number(payment.amount).toLocaleString()}</p>
                            <span className="text-xs font-bold text-[#8696a0]/50  tracking-normal italic">• Sem {payment.semester}</span>
                          </div>
                          <p className="text-xs text-[#8696a0] font-mono mt-1  flex items-center gap-1.5 font-bold">
                ...
                return (
                    <div className="space-y-6">
                        <div key={sem} className="bg-white dark:bg-[#202c33] p-5 sm:p-5 sm:p-6 rounded-3xl shadow-sm border border-slate-50 dark:border-white/5 space-y-8 relative overflow-hidden group">
                            ...
                            <div>
                                <h3 className="font-bold text-slate-900 dark:text-[#e9edef] text-2xl tracking-normal mb-1">Semester {sem} Fees</h3>
                                <p className="text-xs font-bold text-[#8696a0]  tracking-normal flex items-center gap-2">
                            ...
                            <div className="space-y-4 pt-4">
                                ...
                                {pendingAmount > 0 && (
                                    <div className="flex justify-between items-center bg-orange-50 dark:bg-orange-900/10 p-4 rounded-2xl border border-orange-100 dark:border-orange-900/30">
                                        <p className="text-xs font-bold text-orange-500  tracking-normal animate-pulse">Processing Reference</p>
                                        <p className="font-bold text-orange-500 text-xl tracking-normal">₹{pendingAmount.toLocaleString()}</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="pt-8 border-t border-slate-50 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
                                <div className="text-center md:text-left">
                                    ...
                                    <div className="flex items-baseline gap-2">
                                       <p className="text-3xl sm:text-4xl font-bold text-red-600 tracking-normal">₹{dueAmount.toLocaleString()}</p>
                                       <span className="text-sm font-bold text-red-500/50  italic">/ Sem {sem}</span>
                                    </div>
                                </div>
                                <button 
                                    onClick={() => setSelectedSemester(sem)}
                                    className="w-full md:w-auto bg-wa-teal hover:bg-wa-teal/90 text-white px-5 sm:px-8 py-4 rounded-2xl font-bold  tracking-normal text-xs transition-all shadow-xl shadow-wa-teal/20 active:scale-95 group flex items-center justify-center gap-3"
                                >
            ...
            {!paymentMethod ? (
              <div className="space-y-4">
                ...
                >
                  ...
                  <div className="text-left">
                    <p className="font-bold text-slate-900 dark:text-[#e9edef] text-lg tracking-normal ">Cash Register</p>
                    <p className="text-xs font-bold text-[#8696a0]  tracking-normal">Pay directly at front desk</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-left">
                    <p className="font-bold text-slate-900 dark:text-[#e9edef] text-lg tracking-normal ">Digital Sync</p>
                    <p className="text-xs font-bold text-[#8696a0]  tracking-normal">Submit UPI reference</p>
                  </div>
                </button>
              </div>
              ...
              <div className="space-y-8 text-center animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="space-y-3">
                  <p className="text-xs font-bold text-[#8696a0]  tracking-normal">Contact Accounts Manager</p>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <form onSubmit={handleUpload} className="space-y-5 text-left">
                  <div className="group">
                    <label className="block text-xs font-bold text-[#8696a0]  tracking-normal mb-2 ml-1 group-focus-within:text-wa-teal transition-colors">
                      Amount Disburst (₹)
                    </label>
                    <input
                      type="number"
                      ...
                      className="w-full px-5 py-4 bg-[#f0f2f5] dark:bg-[#111b21] border-2 border-transparent focus:border-wa-teal rounded-2xl outline-none text-slate-900 dark:text-[#e9edef] font-bold transition-all"
                      value={paymentAmount}
                  ...
                  <div className="flex gap-4 pt-4">
                    ...
                    <button 
                      ...
                      className="flex-[2] py-4 bg-wa-teal text-white font-bold  tracking-normal text-xs rounded-2xl shadow-xl shadow-wa-teal/20 hover:bg-wa-teal/90 transition-all disabled:bg-slate-300 disabled:shadow-none flex items-center justify-center gap-3 overflow-hidden active:scale-95"
                    >
